...
arma::vec dfun3(arma::vec par, arma::mat betam, int &n, arma::uvec &nJ,
               ...
               arma::vec &indexy, arma::vec &uindexy, arma::vec &y, arma::mat &x)
{
  ...
  for(int i = 0; i < n; i++ )
  {
    ...
    d11 = d11 -0.5/(sigmae2+ n1*sigmav2) - 0.5*1/n1*trans(y(indexi) - x.rows(indexi)* betami)* M1* (y(indexi) - x.rows(indexi)*betami);
    d12 = d12 - 0.5*(1- sigmav2/(sigmae2 +n1*sigmav2))/sigmae2 - 0.5*1/n1*trans(y(indexi) - x.rows(indexi)*betami)*M2*(y(indexi) - x.rows(indexi)*betami);
  }
...
arma::mat Ifun3(arma::vec par, arma::uvec &nJ)
{
  double sigmav2 = par(0);
  double sigmae2 = par(1);
  ...
  I11 = 0.5*sum(nJ1/square(sigmae2 + nJ1*sigmav2));
  I12 = 0.5*sum(1/square(sigmae2 + nJ1*sigmav2));
  I22 = 0.5*sum((1- sigmav2*(2*sigmae2 + nJ1*sigmav2)/square(sigmae2 + nJ1*sigmav2))/pow(sigmae2,2));
...
// [[Rcpp::export]]
Rcpp::List Spgr_bhf3(arma::vec indexy,arma::vec &y,arma::mat &x,
                     ...
                     int maxiter = 100, double tolabs = 1e-4, double tolrel = 1e-2)
{
  ...
  for(m = 0; m < maxiter; m++){
    ...
    for(int i= 0; i < n; i++)
    {
      ...
      arma::mat S1inv = (diag1 - mat1*trans(mat1)*siga2/(sige2 + ni*siga2))/(sige2*ni);
    ...
    for(int j =0; j < 10; j++)
    {
      Imat1 = Ifun3(sig2est, nJ);
      dvalue = dfun3(sig2est, betam, n, nJ, matls, diagls, indexy, uindexy, y, x);
  ...
  for(int i = 0; i < n; i++)
  {
    ...
    arma::mat mat1 = matls[i];
    arma::mat diag1 = diagls[i];
    arma::mat S1 =  mat1*trans(mat1)*sig2est(0) + diag1*sig2est(1);
    arma::mat S1inv = (diag1 - mat1*trans(mat1)*siga2/(sige2 + ni*siga2))/(sige2*ni);
    ...
    loglikvalue = loglikvalue - 0.5*val/ni -
...
// [[Rcpp::export]]
arma::vec loglik3(arma::vec par, arma::vec indexy,arma::vec &y,arma::mat &x,arma::mat &betam,
               List matls, List diagls, arma::uvec nJ)
{
  arma::mat betami;
  arma::vec loglikvalue = zeros(1);
  int n = betam.n_rows;
